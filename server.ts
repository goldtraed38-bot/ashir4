...
async function startServer() {
  ...
  // ─── API سویچ حالت معاملاتی (Quality / Aggressive) ────────────────────────
  app.post("/api/bot/trading-mode-switch", async (req, res) => {
    try {
      const { mode } = req.body; // "quality" | "aggressive"
      if (mode === "quality") {
        // حالت کیفیت: معاملات کم، دقیق، وین‌ریت بالا
        scanner.sensitivity = "conservative";
        scanner.strategy = "strict_elitescalp";
        (scanner as any).tradingModeProfile = "quality";
        await scanner.saveState();
        res.json({ success: true, profile: "quality", message: "✅ حالت کیفیت فعال شد — معاملات کم ولی دقیق" });
      } else if (mode === "aggressive") {
        // حالت تهاجمی: معاملات بیشتر، سود سریع‌تر
        scanner.sensitivity = "active";
        scanner.strategy = "active_goldenscalp";
        (scanner as any).tradingModeProfile = "aggressive";
        await scanner.saveState();
        res.json({ success: true, profile: "aggressive", message: "✅ حالت تهاجمی فعال شد — معاملات بیشتر، سود سریع‌تر" });
      } else {
        res.status(400).json({ success: false, error: "حالت نامعتبر — فقط quality یا aggressive" });
      }
    } catch (e: any) {
      res.status(500).json({ success: false, error: e.message || String(e) });
    }
  ...
  app.get("/api/bot/trading-mode-switch", (req, res) => {
    res.json({ profile: (scanner as any).tradingModeProfile || "quality" });
  });
  // ─────────────────────────────────────────────────────────────────────────────

  // ─── RESET_LOSS_LOCK: ریست قفل از طریق Railway Environment Variable ───────
  // در Railway برو به Variables و بزن: RESET_LOSS_LOCK = true
  // بعد از یه دیپلوی، قفل ریست می‌شه و می‌تونی متغیر رو پاک کنی
  if (process.env.RESET_LOSS_LOCK === "true") {
    try {
      const stateFilePath = path.join(process.cwd(), "ashir_state.json");
      if (fs.existsSync(stateFilePath)) {
        const raw = fs.readFileSync(stateFilePath, "utf-8");
        const state = JSON.parse(raw);
        state.consecutiveLosses = 0;
        state.lossLockUntil = 0;
        state.adaptiveSensitivityOverride = null;
        state.leverageMultiplier = 1.0;
        fs.writeFileSync(stateFilePath, JSON.stringify(state, null, 2));
        console.log("✅ [RESET_LOSS_LOCK] قفل ضرر با موفقیت ریست شد. ربات آماده معامله است.");
      } else {
        console.log("ℹ️ [RESET_LOSS_LOCK] فایل state وجود نداشت — از صفر شروع می‌شه.");
      }
    } catch (resetErr) {
      console.error("❌ [RESET_LOSS_LOCK] خطا در ریست قفل:", resetErr);
    }
  }
  // ─────────────────────────────────────────────────────────────────────────────


  // پشتیبانی از Railway ENV: TRADING_PROFILE = quality | aggressive
  if (process.env.TRADING_PROFILE === "aggressive") {
    scanner.sensitivity = "active";
    scanner.strategy = "active_goldenscalp";
    (scanner as any).tradingModeProfile = "aggressive";
    console.log("🚀 [ENV] حالت تهاجمی از Environment Variable فعال شد.");
  } else if (process.env.TRADING_PROFILE === "quality") {
    scanner.sensitivity = "conservative";
    scanner.strategy = "strict_elitescalp";
    (scanner as any).tradingModeProfile = "quality";
    console.log("🎯 [ENV] حالت کیفیت از Environment Variable فعال شد.");
  }
