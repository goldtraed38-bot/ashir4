...
export default function App() {
  ...
  const [tradingProfile, setTradingProfile] = useState<"quality" | "aggressive">("quality");
  ...
  // ASHIR5 Premium Additions: Fullscreen, Simulated Trade Reset and Live Client-Side Trade Monitoring
  const [isResettingTrades, setIsResettingTrades] = useState(false);

  const handleProfileSwitch = async (mode: "quality" | "aggressive") => {
    setProfileSwitching(true);
    try {
      const r = await fetch("/api/bot/trading-mode-switch", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ mode })
      });
      const d = await r.json();
      if (d.success) setTradingProfile(mode);
    } catch(e) {}
  ...
  // بارگذاری حالت فعلی
  useEffect(() => {
    fetch("/api/bot/trading-mode-switch")
      .then(r => r.json())
      .then(d => { if (d.profile) setTradingProfile(d.profile); })
      .catch(() => {});
  }, []);
  ...
  // Decompose trades into individual trade parts (for counting 50% partial hits & remaining trades closes accurately)
  const getTradeParts = () => {
    const parts: {
      symbol: string;
      action: "buy" | "sell";
      type: "full" | "tp1_partial" | "remaining_50" | "tp1_tp2_combined";
      price_in: number;
      price_out: number;
      value: number;
      pnl_pct: number;
    ...
    // 1. Process active orders (live positions tracking 50% hits)
    if (status.orders) {
      status.orders.forEach(o => {
        if (o.tp1_hit) {
          const pct = (o.action === "buy"
            ? ((o.take_profit_1 - o.entry_price) / o.entry_price) * 100
            : ((o.entry_price - o.take_profit_1) / o.entry_price) * 100) * (o.leverage || 1);
          const val = o.position_value; // remaining 50% position value reflects original/2
          const usd = (pct / 100) * val;
          parts.push({
            symbol: o.symbol,
            action: o.action,
            type: "tp1_partial",
            price_in: o.entry_price,
            price_out: o.take_profit_1,
            value: val,
            pnl_pct: pct,
            pnl_usd: usd,
            time: o.closed_at || Date.now(),
            exitReason: "Take Profit 1 (خروج ۵۰٪ تارگت اول)",
            order: o
          });
        }
      });
    ...
    // 2. Process fully closed orders
    if (status.closedOrders) {
      status.closedOrders.forEach(o => {
        if (o.tp1_hit) {
          // Single compound entry representing the complete unified trade in stats and archives
          parts.push({
            symbol: o.symbol,
            action: o.action,
            type: "tp1_tp2_combined" as any,
            price_in: o.entry_price,
            price_out: o.exit_price || o.entry_price,
            value: o.initial_position_value || (o.position_value * 2),
            pnl_pct: o.pnl_pct || 0,
            pnl_usd: o.pnl_usd || 0,
            time: o.closed_at || Date.now(),
            exitReason: o.exit_reason || "تارگت ۲ یا حد ضرر بریک‌اون",
            order: o
          });
        } else {
          // Full Close (no tp1_hit)
          parts.push({
            symbol: o.symbol,
            action: o.action,
            type: "full",
            price_in: o.entry_price,
            price_out: o.exit_price || o.entry_price,
            value: o.position_value,
            pnl_pct: o.pnl_pct || 0,
            pnl_usd: o.pnl_usd || 0,
            time: o.closed_at || Date.now(),
            exitReason: o.exit_reason || "خروج کامل",
            order: o
          });
        }
      });
  ...
  const tradeParts = getTradeParts();
  const profitParts = tradeParts.filter(p => p.pnl_pct >= 0);
  const lossParts = tradeParts.filter(p => p.pnl_pct < 0);

  const profitTradesCount = profitParts.length;
  const profitTotalPct = profitParts.reduce((sum, p) => sum + p.pnl_pct, 0);
  const profitTotalUsd = profitParts.reduce((sum, p) => sum + p.pnl_usd, 0);
  const profitFullCount = profitParts.filter(p => p.type === "full" || p.type === "remaining_50").length;
  const profitPartialCount = profitParts.filter(p => p.type === "tp1_partial").length;

  const lossTradesCount = lossParts.length;
  const lossTotalPct = lossParts.reduce((sum, p) => sum + p.pnl_pct, 0);
  const lossTotalUsd = lossParts.reduce((sum, p) => sum + p.pnl_usd, 0);
  const lossFullCount = lossParts.filter(p => p.type === "full").length;
  const lossPartialCount = lossParts.filter(p => p.type === "remaining_50").length;
          ...
          <div className="flex items-center gap-8 pr-12 shrink-0">
            ...
            <span className="flex items-center gap-1.5">
              ...
              {lastPriceUpdateAt && Date.now() - lastPriceUpdateAt < 5000 ? (
                ...
                <span className="text-slate-500">در انتظار WebSocket</span>
              )}
            </span>
            <span className="text-slate-700">|</span>
            <span className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-violet-400 animate-pulse" />
              <strong className="text-white">موتور تحلیل:</strong> <span className="text-violet-400">ASHIR5 — 11 عامل + SMC + OB + MSB + HA + ST</span>
            </span>
            <span className="text-slate-700">|</span>
            <span className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
              <strong className="text-white">مدیریت ریسک:</strong> <span className="text-emerald-400">هوشمند (Kelly + GARCH)</span>
            </span>
          </div>
        ...
        <div className="max-w-[1720px] mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <div className="text-right">
                <span className="text-white font-display font-black text-lg tracking-wider block leading-none">
                  ASHIR<span className="text-[#F0B90B]">5</span>
                </span>
            ...
            <div className="hidden md:flex items-center gap-3 bg-black/40 border border-[#2B3139] px-3 py-1.5 rounded-lg text-[10px] font-mono text-slate-400 mr-2">
              ...
              <span className="text-slate-700">|</span>
              <span>استراتژی: <strong className="text-cyan-400 font-display">{status.strategy === "active_goldenscalp" ? "SMC GOLDEN SCALP v5" : "ELITE SMC STRICT v5"}</strong></span>
              <span className="text-slate-700">|</span>
          ...
          <div className="hidden lg:flex items-center gap-2.5 bg-[#0C0D10]/95 px-4 h-10 rounded-xl border border-[#2B3139] shadow-inner select-none">
            ...
            <span className="text-[10px] text-slate-500 font-bold border-r border-[#2B3139] pr-2.5 h-4 flex items-center">
              ASHIR5 • Order Block + MSB + FVG + SuperTrend • لوریج ۱۰-۵۰x
            </span>
          </div>
            ...
            >
              ...
              <div className="max-w-4xl text-right space-y-3 relative z-10">
                <h3 className="text-sm font-black text-white flex items-center gap-2">
                  ...
                  راهنمای راهبری و کاربری ربات اشیر ۵.۰ کورتکس صرافی XT.com
                </h3>
                <p className="text-xs text-slate-300 leading-relaxed">
                  ربات <strong className="text-[#F0B90B]">اشیر ۵.۰</strong> با موتور تحلیل ۱۱ عاملی SMC ارتقا یافته است. این نسخه Order Block، Market Structure Break، Fair Value Gap، Heikin Ashi و SuperTrend را به موتور قبلی اضافه کرده. اتصال <strong>WebSocket لحظه‌ای (~۲۰ms)</strong> جایگزین polling شده تا قیمت در کسری از ثانیه اعمال شود. ربات <strong>فقط</strong> وقتی همگرایی ۶+ عامل از ۱۱ عامل در یک جهت باشد وارد معامله می‌شود.
                </p>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-1.5 text-xs">
                  ...
                  <div className="bg-black/30 p-3.5 rounded-lg border border-white/5 space-y-1">
                    ...
                    <p className="text-slate-400 text-[11px] leading-relaxed">موتور ۱۱ عاملی ASHIR5: Order Block، Market Structure Break، Fair Value Gap، Heikin Ashi، SuperTrend، VWAP، GARCH، Iceberg Hunter و Volume POC همگی در یک موتور اجماع قدرتمند ادغام شده‌اند.</p>
                  </div>
                </div>
              </div>
            </motion.section>
        ...
        {activeTab === "cockpit" && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
            ...
            <div className="lg:col-span-3 space-y-4">
              ...
              {/* 🎯 سویچ حالت معاملاتی */}
              <section className="bg-[#181A20] border border-[#2B3139] rounded-xl p-4 space-y-3 shadow-lg">
                <div className="flex items-center justify-between border-b border-[#2B3139] pb-2">
                  <div className="flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 bg-purple-400 rounded-full animate-pulse" />
                    <h3 className="text-xs font-black text-white">حالت معاملاتی (Trading Profile)</h3>
                  </div>
                  <span className={cn(
                    "text-[8px] font-black px-1.5 py-0.5 rounded",
                    tradingProfile === "quality" ? "bg-blue-500/20 text-blue-400" : "bg-orange-500/20 text-orange-400"
                  )}>
                    {tradingProfile === "quality" ? "🎯 QUALITY" : "🚀 AGGRESSIVE"}
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <button
                    onClick={() => handleProfileSwitch("quality")}
                    disabled={profileSwitching}
                    className={cn(
                      "h-16 rounded-xl flex flex-col items-center justify-center gap-1 text-xs font-black border transition-all",
                      tradingProfile === "quality"
                        ? "bg-blue-500/20 border-blue-400 text-blue-300"
                        : "bg-black/20 border-[#2B3139] text-slate-400 hover:border-blue-400/50"
                    )}
                  >
                    <span className="text-lg">🎯</span>
                    <span>کیفیت</span>
                    <span className="text-[9px] font-normal opacity-70">معامله کم، دقیق</span>
                  </button>
                  <button
                    onClick={() => handleProfileSwitch("aggressive")}
                    disabled={profileSwitching}
                    className={cn(
                      "h-16 rounded-xl flex flex-col items-center justify-center gap-1 text-xs font-black border transition-all",
                      tradingProfile === "aggressive"
                        ? "bg-orange-500/20 border-orange-400 text-orange-300"
                        : "bg-black/20 border-[#2B3139] text-slate-400 hover:border-orange-400/50"
                    )}
                  >
                    <span className="text-lg">🚀</span>
                    <span>تهاجمی</span>
                    <span className="text-[9px] font-normal opacity-70">معامله بیشتر، سریع</span>
                  </button>
                </div>
                <p className="text-[9px] text-slate-500 text-center">
                  {tradingProfile === "quality" ? "۱۰/۱۴ فیلتر · ML ≥۷۴٪ · RR=2.2 — وین‌ریت بالا" : "۷/۱۴ فیلتر · ML ≥۶۰٪ · RR=1.8 — معاملات بیشتر"}
                </p>
              </section>
