...
export class WaterfallScanner {
  ...
  public lossLockUntil: number = 0; // timestamp — بعد از این وقت قفل conservative رفع می‌شه
...
🐉 <b>سامانه مدیریت دارایی اشیر ۴.۰</b> | ${new Date().toLocaleTimeString("fa-IR")}
  ...
  constructor(client: XTClient, reporter: TelegramReporter, config: Config) {
    ...
    // 🔌 WebSocket real-time price connection
  ...
  public async saveState() {
    ...
    try {
      const state = {
        ...
        lossLockUntil: this.lossLockUntil,
  ...
  private async loadState() {
    try {
      ...
      this.lossLockUntil = state.lossLockUntil || 0;
  ...
  public calculateCortexDynamicSensitivity(): "conservative" | "balanced" | "active" {
    // 0. Auto-reset قفل ضرر بعد از ۲ ساعت — جلوگیری از قفل ابدی
    if (this.lossLockUntil > 0 && Date.now() > this.lossLockUntil) {
      this.consecutiveLosses = 0;
      this.lossLockUntil = 0;
      this.adaptiveSensitivityOverride = null;
      this.leverageMultiplier = 1.0;
      this._addLog("🔓 [Auto-Reset] قفل ضرر ۲ ساعته منقضی شد. سیستم به حالت balanced بازگشت.");
    }

    // 1. بعد از ۳ ضرر متوالی قفل می‌کنه (قبلاً ۲ بود — خیلی حساس)
    ...
    if (recentTrades.length < 4) {
      return "balanced"; // شروع با حالت متعادل تا ربات فرصت معامله داشته باشد
    }
    ...
    if (winRate >= 0.65) {
      return "active"; // با ۶۵٪+ نرخ برد = active
    } else if (winRate < 0.45) {
      return "balanced"; // حتی با وین‌ریت پایین هم balanced بمون — نه conservative
    }
    return "balanced";
  }

  public calculateCortexDynamicStrategy(): "strict_elitescalp" | "active_goldenscalp" {
    // 1. بعد از ۲ ضرر متوالی به strict_elitescalp برو (قبلاً ۱ بود — خیلی حساس)
    ...
    // 3. ASHIR5: Default to strict until proven ≥70% win rate track record
    const recentTrades = this.closedOrders.slice(0, 6);
    if (recentTrades.length < 4) {
      return "strict_elitescalp"; // شروع همیشه با سخت‌گیری — نه active
    }
    ...
    if (winRate >= 0.70) {
      // فقط با ۷۰٪+ نرخ برد اجازه active_goldenscalp داده می‌شود
              ...
              if (corrCheck.allowed && corrCheck.adjusted_size > 0) {
                ...
                if (isRealOrderSuccess) {
                  ...
                  if (!this.orders.find(o => o.symbol === order.symbol)) {
                    ...
                    // 🔌 Subscribe to real-time WS price for this position
                    this.client.wsSubscribe(order.symbol);
  ...
  private async runHighFrequencyLiveTracker() {
    ...
    while (this.isRunning) {
      ...
      // Reduced to 350ms — WS cache makes getLivePrice instant (no network wait)
...
🐉 <b>سامانه هوشمند اشیر ۴.۰</b> | ${new Date().toLocaleTimeString("fa-IR")}
    ...
    // تنظیم زمان انقضای قفل: ۲ ساعت از آخرین ضرر
    this.lossLockUntil = Date.now() + 2 * 60 * 60 * 1000;
    ...
    // به جای قفل کردن، فقط حجم معامله رو کم کن — ربات همیشه کار می‌کنه
    let newSensitivity: "conservative" | "balanced" | "active" = "balanced";
    if (this.consecutiveLosses >= 5) {
      newSensitivity = "conservative"; // فقط بعد از ۵ ضرر متوالی محافظه‌کار بشو
    }
    // override رو null نگه‌دار تا cortex خودش تصمیم بگیره (قفل نمی‌شه)
    this.adaptiveSensitivityOverride = null;

    // Scale down leverage
    this.leverageMultiplier = Math.max(0.70, 1.0 - (this.consecutiveLosses * 0.08)); // کاهش ملایم‌تر اهرم
    ...
    const riskWindow = 45 * 60 * 1000; // 45 دقیقه — بازگشت سریع‌تر به حالت عادی
