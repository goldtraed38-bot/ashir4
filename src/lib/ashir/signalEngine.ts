...
import { VolRegimeStrategy, LiquidityStrategy, FundingStrategy, CorrelationStrategy, TimeSniperStrategy, AdvancedConfluenceScalper, MicroScalpStrategy, FairValueGapDetector, VolumePOC } from "./strategies";
...
export class SignalEngine {
  ...
  private microScalp = new MicroScalpStrategy();
  private fvgDetector = new FairValueGapDetector();
  private volumePOC = new VolumePOC();
  ...
  public minScore = 0.91;

  // 🧠 Smart Risk-Reduction: extra win-probability the engine must clear before
  // entering a trade on this symbol (set temporarily by the scanner after a loss,
  // decaying back to 0 — replaces the old hard quarantine block).
  public extraMinConfidence = 0;

  private getDynamicThreshold(regime: string): number {
    // آستانه‌های واقعی — نه خیلی سخت، نه خیلی شل
    if (this.sensitivity === "conservative") {
      if (regime === "extreme") return 0.80;
      if (regime === "high") return 0.77;
      if (regime === "normal") return 0.74;
      if (regime === "low") return 0.72;
      return 0.74;
    } else if (this.sensitivity === "active") {
      if (regime === "extreme") return 0.72;
      if (regime === "high") return 0.69;
      if (regime === "normal") return 0.66;
      if (regime === "low") return 0.64;
      return 0.66;
    } else { // balanced
      if (regime === "extreme") return 0.76;
      if (regime === "high") return 0.73;
      if (regime === "normal") return 0.70;
      if (regime === "low") return 0.67;
      return 0.70;
    }
  }
  ...
  async analyze(symbol: string, klines: Klines, orderbook: OrderBook | null, change24h = 0, btcChange = 0, livePrice = 0): Promise<Signal | null> {
    ...
    // 🆕 ASHIR 5.0: Fair Value Gap + Volume POC
    const fvgResult = this.fvgDetector.detect(opens, highs, lows, closes);
    const pocResult = this.volumePOC.analyze(highs, lows, closes, volumes, 20);

    // استراتژی اسکالپ بسیار سریع (رای مستقل، مبتنی بر کراس EMA + شکست رنج کوچک + RSI(7))
    const microScalpResult = this.microScalp.analyze(closes, highs, lows, orderbook);

    const atr = ATR.calculate(highs, lows, closes, 14);
    const relativeATR = atr / currentPrice;

    // ==========================================
    // 🛡️ RE-ARCHITECTED 14 STRICT FILTER CORES (incl. Micro-Scalp confirmation)
    // ==========================================
    ...
    const longFilters = [
      {
        ...
        passed: currentPrice > latestEma21 && currentPrice > latestEma50 && (latestEma9 > latestEma21 || ema50SlopePositive),
        desc: `قیمت (${currentPrice.toFixed(4)}) بالای EMA21 (${latestEma21.toFixed(4)}) و EMA50 (${latestEma50.toFixed(4)}) — ترند صعودی تایید شد.`
      },
      {
        id: 2,
        key: "rsi_safety",
        name: "محدوده سلامت مومنتوم (RSI_SH)",
        passed: rsiVal >= 40.0 && rsiVal <= 70.0,
        desc: `شاخص RSI (${rsiVal.toFixed(1)}) در محدوده مناسب معامله (بین ۴۰ تا ۷۰) قرار دارد.`
      },
      {
        id: 3,
        key: "mfi_flow",
      ...
      },
      {
        ...
        passed: volumeSurgeRatio >= 0.90 || volSurgeResult.surge,
        desc: `نسبت حجم معاملات (${volumeSurgeRatio.toFixed(2)}) در محدوده قابل قبول.`
      },
      {
        id: 6,
        ...
        desc: `برتری خفیف یا بیشتر صف خرید اوردربوک: +${(imbalance * 100).toFixed(1)}%.`
      },
      {
        id: 7,
        key: "range_breakout",
      ...
      {
        id: 14,
        key: "micro_scalp_confirm",
        name: "تایید اسکالپ سریع چندعاملی (MSCC)",
        passed: microScalpResult.signal !== "buy",
        desc: microScalpResult.reason
      }
    ];
    ...
    const shortFilters = [
      {
        ...
        name: "پایش همگرایی چندزمانی خطوط EMA (MTCG)",
        passed: currentPrice < latestEma21 && currentPrice < latestEma50 && (latestEma9 < latestEma21 || !ema50SlopePositive),
        desc: `قیمت (${currentPrice.toFixed(4)}) زیر EMA21 (${latestEma21.toFixed(4)}) و EMA50 (${latestEma50.toFixed(4)}) — ترند نزولی تایید شد.`
      },
      {
        id: 2,
        key: "rsi_safety",
        name: "محدوده سلامت مومنتوم (RSI_SH)",
        passed: rsiVal >= 30.0 && rsiVal <= 60.0,
        desc: `شاخص RSI (${rsiVal.toFixed(1)}) در محدوده ریزشی مناسب (بین ۳۰ تا ۶۰) قرار دارد.`
      },
      {
        id: 3,
        key: "mfi_flow",
      ...
      },
      {
        ...
        key: "garch_vol",
        name: "سقف نوسانات آماری GARCH (VGCC)",
        passed: volMetrics.annualized_vol < 1.80 && volMetrics.regime !== "extreme",
        desc: `نوسان سالانه (${(volMetrics.annualized_vol * 100).toFixed(1)}%) زیر سقف ۱۸۰٪.`
      },
      {
        id: 5,
        key: "vol_surge_check",
      ...
      },
      {
        ...
        passed: imbalance <= -0.01,
        desc: `برتری خفیف یا بیشتر صف فروش اوردربوک: ${(imbalance * 100).toFixed(1)}%.`
      },
      {
        id: 7,
        key: "range_breakout",
      ...
      },
      {
        ...
        key: "micro_scalp_confirm",
        name: "تایید اسکالپ سریع چندعاملی (MSCC)",
        passed: microScalpResult.signal !== "sell",
        desc: microScalpResult.reason
      }
    ];
    ...
    if (confResult.signal === "buy") {
      ...
      if (longPassedCount >= shortPassedCount) {
        finalScore = Math.min(0.70, longPassedCount / 14);
      } else {
        finalScore = Math.max(0.30, 1 - (shortPassedCount / 14));
      }
    }

    // ── 🔒 دروازه اجباری فیلترهای ۱۴گانه ─────────────────────────────────
    // این بخش بخش اصلی جلوگیری از ضرر است.
    // برای ورود به معامله باید:
    //   - خرید: حداقل ۱۱ از ۱۴ فیلتر long پاس شده باشند
    //   - فروش: حداقل ۱۱ از ۱۴ فیلتر short پاس شده باشند
    // این یعنی فقط معاملاتی که از تقریباً همه فیلترها رد شوند وارد می‌شوند
    // 🎯 حالت کیفیت (conservative): 10/14 فیلتر — معاملات نادر ولی دقیق
    // 🚀 حالت تهاجمی (active): 7/14 فیلتر — معاملات بیشتر
    const REQUIRED_LONG_FILTERS = this.sensitivity === "conservative" ? 10 : this.sensitivity === "active" ? 7 : 8;
    const REQUIRED_SHORT_FILTERS = this.sensitivity === "conservative" ? 10 : this.sensitivity === "active" ? 7 : 8;

    if (action === "buy" && longPassedCount < REQUIRED_LONG_FILTERS) {
      const failedFilters = longFilters.filter(f => !f.passed).map(f => f.name).join("، ");
      vetoReason = `🚫 رد ورود: تنها ${longPassedCount}/14 فیلتر خرید تایید شد (حداقل ${REQUIRED_LONG_FILTERS} نیاز است). فیلترهای رد شده: ${failedFilters}`;
      action = "stay_out";
      finalScore = 0.5;
    } else if (action === "sell" && shortPassedCount < REQUIRED_SHORT_FILTERS) {
      const failedFilters = shortFilters.filter(f => !f.passed).map(f => f.name).join("، ");
      vetoReason = `🚫 رد ورود: تنها ${shortPassedCount}/14 فیلتر فروش تایید شد (حداقل ${REQUIRED_SHORT_FILTERS} نیاز است). فیلترهای رد شده: ${failedFilters}`;
      action = "stay_out";
      finalScore = 0.5;
    }

    // ── فیلتر FVG + POC اضافی ─────────────────────────────────────────────
    // اگر هیچ FVG یا POC نزدیکی وجود نداشته باشد، احتمال برد پایین است
    if (action !== "stay_out") {
      const fvg = (this as any).fvgDetector?.detect(opens, highs, lows, closes);
      const poc = (this as any).volumePOC?.analyze(highs, lows, closes, volumes, 20);
      const hasFVGSupport = fvg?.signal === action;
      const hasPOCSupport = poc?.signal === action;
      if (!hasFVGSupport && !hasPOCSupport) {
        // هیچ تایید SMC اضافی وجود ندارد — اگر اطمینان کم باشد رد می‌شود
        if (confResult.score < 0.65) {
          vetoReason = `🚫 رد ورود: بدون تایید FVG یا Volume POC در جهت سیگنال. Score: ${(confResult.score * 100).toFixed(0)}٪ زیر حداقل ۶۵٪ برای ورود بدون SMC support.`;
          action = "stay_out";
          finalScore = 0.5;
        }
      }
    ...
    const rawStopLossPct = Math.max(1.4 * (atr / currentPrice), 0.010); 
    const stopLossPct = Math.min(rawStopLossPct, 0.025); // cap at 2.5% — ضرر محدودتر

    // 🎯 حالت کیفیت: RR بالاتر — هر معامله سود بیشتری داره
    // 🚀 حالت تهاجمی: RR کمتر ولی سیگنال‌های بیشتر
    const isQualityMode = this.sensitivity === "conservative";
    let dynamicRR1 = isQualityMode ? 2.2 : 1.8;
    let dynamicRR2 = isQualityMode ? 4.0 : 3.0;
    ...
    dynamicRR1 = Math.max(1.80, Math.min(2.50, dynamicRR1));  // حداقل ۱.۸، حداکثر ۲.۵
    dynamicRR2 = Math.max(3.00, Math.min(5.00, dynamicRR2));  // حداقل ۳، حداکثر ۵
    ...
    let minMLConfidence = 0.70;
    if (this.sensitivity === "conservative") {
      // 🎯 حالت کیفیت: فقط بهترین سیگنال‌ها — وین‌ریت بالا
      minMLConfidence = 0.74;
    } else if (this.sensitivity === "balanced") {
      minMLConfidence = 0.68;
    } else {
      // 🚀 حالت تهاجمی: معاملات بیشتر — سود سریع‌تر
      minMLConfidence = 0.60;
    }

    const effectiveMinMLConfidence = Math.min(0.94, minMLConfidence + this.extraMinConfidence);

    if (action !== "stay_out" && mlConfidence < effectiveMinMLConfidence) {
      const extraNote = this.extraMinConfidence > 0
        ...
        : "";
      vetoReason = `Cortex Predictive Setup Reject: همگرایی و تایید آماری کورتکس بر روی الگوهای معاملاتی (${(mlConfidence * 100).toFixed(1)}٪) از حداقل حدنصاب بهینه‌سازی شده پس از زیان‌های اخیر (${(effectiveMinMLConfidence * 100).toFixed(1)}٪)${extraNote} کمتر است. ربات برای پیشگیری از ثبت زیان مکرر، ورود را به طور خودکار لغو نمود.`;
      action = "stay_out";
    ...
    // ⚡ Smart Adaptive Leverage Selector (10x to 50x) — مخصوص اسکالپ سریع
    // اهرم مستقیماً بر اساس "احتمال برد" واقعی (mlConfidence) تعیین می‌شود؛
    // هرچه اطمینان سیستم به موفقیت معامله بیشتر باشد، اهرم بالاتر می‌رود.
    // نوسان بازار (ATR نسبی) به‌عنوان ضریب اصلاح ایمنی روی این عدد اعمال می‌شود
    // تا در شرایط پرنوسان، حتی با اعتماد بالا، اهرم به‌صورت غیرمنطقی بزرگ نشود.
    // ── ASHIR5: اهرم امن و محافظه‌کارانه ───────────────────────────────────
    // اهرم حداکثر 20x — بالاتر از این ریسک را به شدت افزایش می‌دهد
    // برای هر ۵٪ اطمینان بیشتر از کف، یک اهرم بالاتر
    let leverageSelection = 10;
    if (action !== "stay_out") {
      const confidenceSpan = Math.max(0.0001, 0.95 - effectiveMinMLConfidence);
      const normalizedConfidence = Math.max(0, Math.min(1, (mlConfidence - effectiveMinMLConfidence) / confidenceSpan));

      // نوسان بالا = اهرم کمتر
      const volRiskDampener = Math.max(0.3, Math.min(1.0, 0.010 / relativeATR));

      // حداکثر 20x (نه 50x) — محافظت از سرمایه اولویت اول
      const rawLeverage = 10 + normalizedConfidence * 10; // 10x .. 20x
      leverageSelection = Math.round(rawLeverage * volRiskDampener);
      leverageSelection = Math.max(10, Math.min(20, leverageSelection));
    }
